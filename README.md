...
Biome OCR reads the foreground, maximized Roblox window at the **1920 × 1080** or **2560 × 1440** resolution selected in Settings. Scanning pauses when you switch apps and resumes when Roblox returns to foreground. In the current development trial, one shared in-memory capture feeds **Tesseract** for the tight biome row and Windows OCR for the existing Play marker. Tesseract runs as one persistent local worker with bundled English data, using single-line recognition. It tries the original crop, then contrast-stretched red, green, and blue channels until a biome reaches 70% text similarity. The fuzzy name matcher remains separate from Tesseract's own confidence score. No failed candidate is displayed. Each cycle finishes with a **500 ms wait**, so capture and OCR work add to the effective interval. No screenshots are saved or uploaded by the live pipeline.
...
Grab the [v0.5.3 Windows x64 ZIP](https://github.com/SamSKNg/SolSeer/releases/download/v0.5.3/solseer-v0.5.3-windows-x64.zip). See the [v0.5.3 release notes](https://github.com/SamSKNg/SolSeer/releases/tag/v0.5.3) for what changed. When updating, stop the old copy and extract the new ZIP into a fresh folder; your saved cookie, notification preferences, join history, and biome observations stay in their separate local settings folder. Join attempts from older sessions that were never persisted cannot be recovered.
...
Tesseract receives only the tight biome crop, including for color-channel fallback: `(4,860,320,25)` at 1080p and `(5,1150,425,32)` at 1440p (x, y, width, height). The right margin accommodates longer names. Version, time-of-day and currency text are excluded; Play keeps the shared full-color Windows pass. New history observations identify their source as `tesseract`; older `windows_ocr` records remain readable.
